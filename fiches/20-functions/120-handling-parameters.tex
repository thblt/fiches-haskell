...
Pour employer des constantes nommées, on peut utiliser un préprocesseur \qsee{preprocessor} ou un type algébrique \qsee{algebraic-types}.
\end{warnbox}
...
Un garde est une expression de type \hsT{Bool}. Si l'expression s'évalue à \hs{True}, l'implémentation qui suit est utilisée. L'usage de gardes permet souvent d'éviter les constructions conditionnelles avec \hs{if} \qsee{if-then-else}.
